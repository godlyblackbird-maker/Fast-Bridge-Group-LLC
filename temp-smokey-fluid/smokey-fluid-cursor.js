const DEFAULT_BLACKLIGHT_FLUID_CONFIG = {
    simResolution: 128,
    dyeResolution: 1024,
    ...
    transparent: true,
    id: "fast-blacklight-fluid-cursor"
};
const BLACKLIGHT_FLUID_STYLE_ID = "fast-blacklight-fluid-cursor-style";
const BLACKLIGHT_FLUID_PALETTE = [
    { r: 255, g: 84, b: 196 },
    { r: 28, g: 129, b: 46 },
    { r: 255, g: 106, b: 31 },
    { r: 182, g: 228, b: 255 },
...
function normalizeHexColor(color, fallback = "#ff54c4") {
    const normalized = String(color || "").trim();
    return /^#[0-9a-fA-F]{6}$/.test(normalized) ? normalized.toLowerCase() : fallback;
}
function normalizeBrightness(value, fallback = 1) {
    const numeric = Number.parseFloat(String(value ?? "").trim());
    if (!Number.isFinite(numeric)) {
        return fallback;
    }
    return Math.min(2.2, Math.max(0.2, numeric));
}
function hexToFluidColor(color, brightness = 1) {
    const normalized = normalizeHexColor(color);
    const intensity = 0.22 * normalizeBrightness(brightness, 1);
    return {
        r: (Number.parseInt(normalized.slice(1, 3), 16) / 255) * intensity,
        g: (Number.parseInt(normalized.slice(3, 5), 16) / 255) * intensity,
        b: (Number.parseInt(normalized.slice(5, 7), 16) / 255) * intensity
    };
}
export function mountBlacklightFluidCursor(incomingConfig = {}) {
    const config = {
        ...DEFAULT_BLACKLIGHT_FLUID_CONFIG,
        ...incomingConfig,
        backColor: {
            ...DEFAULT_BLACKLIGHT_FLUID_CONFIG.backColor,
            ...(incomingConfig.backColor || {})
        },
        colorMode: String(incomingConfig.colorMode || DEFAULT_BLACKLIGHT_FLUID_CONFIG.colorMode).trim().toLowerCase() === "solid" ? "solid" : "dynamic",
        brightness: normalizeBrightness(incomingConfig.brightness, DEFAULT_BLACKLIGHT_FLUID_CONFIG.brightness),
        solidColor: normalizeHexColor(incomingConfig.solidColor || DEFAULT_BLACKLIGHT_FLUID_CONFIG.solidColor)
    };
    let canvas = document.getElementById(config.id);
    if (!(canvas instanceof HTMLCanvasElement)) {
        canvas = document.createElement("canvas");
        canvas.id = config.id;
        canvas.className = "blacklight-fluid-cursor-canvas";
        canvas.setAttribute("aria-hidden", "true");
        document.body.appendChild(canvas);
    }
    return initFluid(config);
}
/**
...
export const initFluid = (config) => {
    ...
    const style = document.getElementById(BLACKLIGHT_FLUID_STYLE_ID) || document.createElement("style");
    style.id = BLACKLIGHT_FLUID_STYLE_ID;
    style.textContent = `
    #${config.id} {
      position: fixed;
      top: 0;
  ...
  `;
    ...
    function addWindowListener(type, handler, options) {
        ...
        cleanupCallbacks.push(() => window.removeEventListener(type, handler, options));
    }
    ...
    class PointerPrototype {
        constructor() {
            ...
            this.texcoordX = 0; // Normalized X coordinate (0-1)
            this.texcoordY = 0; // Normalized Y coordinate (0-1)
            this.prevTexcoordX = 0; // Previous normalized X coordinate
            this.prevTexcoordY = 0; // Previous normalized Y coordinate
            this.deltaX = 0; // X movement delta
            this.deltaY = 0; // Y movement delta
            this.down = false; // Whether pointer is currently pressed
            this.moved = false; // Whether pointer has moved since last update
            this.color = [30, 0, 300]; // RGB color for fluid emission
        }
    }
    ...
    class Material {
        ...
        setKeywords(keywords) {
            ...
            if (!program) {
                ...
                const fragmentShader = compileShader(gl, gl.FRAGMENT_SHADER, addKeywords(this.fragmentShaderSource, keywords));
    ...
    const advectionShader = compileShader(gl, gl.FRAGMENT_SHADER, addKeywords(`
      precision highp float;
      precision highp sampler2D;
    ...
    let dye; // Color/density field
    let velocity; // Velocity field
    let divergence; // Divergence field
    let curl; // Vorticity field
    let pressure; // Pressure field
    // Shader programs
    ...
    function updateColors(dt) {
        if (config.colorMode === "solid" || config.colorUpdateSpeed <= 0) {
            const solidColor = rgbToTuple(hexToFluidColor(config.solidColor, config.brightness));
            pointers.forEach((p) => {
                p.color = solidColor;
            });
            return;
    ...
    addWindowListener("touchstart", handleTouchStart);
    addWindowListener("touchmove", handleTouchMove, { passive: false });
    ...
    function generateColor() {
        if (config.colorMode === "solid") {
            return hexToFluidColor(config.solidColor, config.brightness);
        }
        const swatch = BLACKLIGHT_FLUID_PALETTE[Math.floor(Math.random() * BLACKLIGHT_FLUID_PALETTE.length)] || BLACKLIGHT_FLUID_PALETTE[0];
        const intensity = 0.22 * config.brightness;
        return {
            r: (swatch.r / 255) * intensity,
            g: (swatch.g / 255) * intensity,
            b: (swatch.b / 255) * intensity
        };
    }
    /**
